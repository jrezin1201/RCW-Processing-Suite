...
def process_lennar_file(job_id: str, filepath: str, original_filename: str = None) -> None:
    ...
    try:
        ...
        # Log the full traceback server-side for debugging but return a
        # generic message to the client — tracebacks can reveal internal
        # paths, library versions, and row-level data.
        logger.exception("Lennar processing failed for job %s", job_id)
        set_job(job_id, {
            "job_id": job_id,
            "status": "failed",
            "progress": 0.0,
